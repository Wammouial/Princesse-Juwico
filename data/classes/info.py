...
class OverheadInfo(object):
    ...
    def __init__(self, game_info, state):
        ...
        self.level_number = game_info[c.LEVEL_NUMBER]
        self.state = state
        ...
        self.create_flashing_bad()
        self.create_juwico_image()
    ...
    def create_info_labels(self):
        ...
        self.create_label(self.juwico_label, 'JUWICO', 75, 30)
        self.create_label(self.world_label, 'WORLD', 450, 30)
        self.create_label(self.time_label, 'TIME', 625, 30)
        if self.level_number == '1-1':
            self.create_label(self.stage_label, '1-1', 472, 55)
        elif self.level_number == '1-2':
            self.create_label(self.stage_label, '1-2', 472, 55)
        elif self.level_number == '1-3':
            self.create_label(self.stage_label, '1-3', 472, 55)
        elif self.level_number == '1-4':
            self.create_label(self.stage_label, '1-4', 472, 55)
        elif self.level_number == '0-0':
            self.create_label(self.stage_label, '0-0', 472, 55)
        elif self.level_number == '0-1': 
            self.create_label(self.stage_label, '0-1', 472, 55)

        self.label_list = [self.juwico_label,
                           self.world_label,
                           self.time_label,
                           self.stage_label]
    ...
    def create_load_screen_labels(self):
        ...
        if self.level_number == '1-1':
            ...
            self.create_label(number_label, '1-2', 430, 200)
        elif self.level_number == '1-3':
            self.create_label(number_label, '1-3', 430, 200)
        elif self.level_number == '1-4':
            self.create_label(number_label, '1-4', 430, 200)
        elif self.level_number == '0-0':
            self.create_label(number_label, '0-0', 430, 200)
        elif self.level_number == '0-1': 
            self.create_label(number_label, '0-1', 430, 200)
    ...
    def create_label(self, label_list, string, x, y):
        """Creates a label (WORLD, TIME, JUWICO)"""
    ...
    def create_coin_counter(self):
        """Creates the info that tracks the number of coins Juwico collects"""
        self.coin_count_images = []
        self.create_label(self.coin_count_images, '*00', 310, 45)


    def create_flashing_bad(self):
        """Creates the flashing coin next to the coin total"""
        self.flashing_bad = flashing_bad.Bad(265, 10)


    def create_juwico_image(self):
        """Get the juwico image"""
        ...
        self.juwico_image = self.get_image(104,5,12,18)
        self.juwico_rect = self.juwico_image.get_rect(center=(320, 290))
    ...
    def create_main_menu_labels(self):
        ...
        self.create_label(player_one_game, 'LANCER LE JEU', 272, 360)
        self.create_label(help, 'AIDE', 272, 405)
        self.create_label(top, 'TOP - ', 290, 465)
        self.create_label(top_score, '000000', 400, 465)

        self.main_menu_labels = [player_one_game, help,
                                 top, top_score]
                                 
    def update(self, level_info, juwico=None):
    ...
    def handle_level_state(self, level_info):
        ...
        if self.state == c.MAIN_MENU:
            ...
            self.flashing_bad.update(level_info[c.CURRENT_TIME])
        ...
        elif self.state == c.LEVEL1 or self.state == c.LEVEL2 or self.state == c.LEVEL3 or self.state == c.LEVEL4 or self.state == c.LEVEL0:
            self.score = level_info[c.SCORE]
            self.update_score_images(self.score_images, self.score)
    ...
    def draw_main_menu_info(self, surface):
        ...
        surface.blit(self.flashing_bad.image, self.flashing_bad.rect)
    ...
    def draw_loading_screen_info(self, surface):
        ...
        surface.blit(self.juwico_image, self.juwico_rect)
